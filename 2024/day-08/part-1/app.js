...
const input = fs.readFileSync('./2024/day-08/part-1/input.txt', 'utf-8');
	...
	for ( let i = 0; i < freq.length; i++ ){
		for ( let j = 0; j < freq.length; j++ ){
			if ( i != j){
				...
				let antinodeRow = freq[i].row + rowDiff

				let colDiff = freq[i].col - freq[j].col;
				let antinodeCol = freq[i].col + colDiff

				if ( antinodeRow >= 0 && antinodeRow < antinodeMap[0].length && antinodeCol >= 0 && antinodeCol < antinodeMap[0].length) {
